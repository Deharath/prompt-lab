  ...
  const testJobs = [
    {
      ...
      output: 'This is a technical document about database performance metrics. It contains factual information about system performance. The report covers various aspects of database optimization.',
      provider: 'openai',
      model: 'gpt-4.1-mini',
    ...
    },
    {
      ...
      output: 'This laptop is absolutely amazing! I love it so much! Best purchase ever made! Incredible performance and fantastic quality!',
      provider: 'openai',
      model: 'gpt-4.1-mini',
      metrics: ['sentiment', 'sentiment_detailed'],
    },
    {
      ...
      output: 'This product is absolutely terrible! I hate it completely! Worst purchase ever! Horrible quality and awful performance!',
      provider: 'openai',
      model: 'gpt-4.1-mini',
      metrics: ['sentiment', 'sentiment_detailed'],
    },
  ...
  for (const testJob of testJobs) {
    ...
    try {
      ...
      if (jobResponse.ok) {
            ...
            if (key.includes('sentiment')) {
              ...
              if (typeof value === 'object' && value !== null) {
                if ('label' in value && 'confidence' in value) {
                  ...
                  console.log(`   -> Distribution: Pos: ${(positive * 100).toFixed(1)}%, Neg: ${(negative * 100).toFixed(1)}%, Neu: ${(neutral * 100).toFixed(1)}%`);
                  ...
                  console.log(`   -> ${correct ? '✅' : '❌'} Label matches highest score (expected: ${expectedLabel})`);
                }
              }
            }
          });
        ...
        console.error('❌ Failed to fetch job details:', jobResponse.status, jobResponse.statusText);
      }
