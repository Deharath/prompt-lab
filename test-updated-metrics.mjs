...
async function testUpdatedMetrics() {
  console.log(
    'Testing updated metrics with DistilBERT and text-readability...\n',
  );
  ...
  for (const text of testTexts) {
    ...
    try {
      ...
      console.log(
        `  Fast (VADER): ${fastResult.compound.toFixed(3)} [${fastResult.mode}]`,
      );
      ...
      console.log(
        `  Accurate (DistilBERT): ${accurateResult.compound.toFixed(3)} [${accurateResult.mode}]`,
      );
      ...
      console.log(
        `  Flesch Reading Ease: ${readability.fleschReadingEase.toFixed(1)}`,
      );
      console.log(
        `  Flesch-Kincaid Grade: ${readability.fleschKincaid.toFixed(1)}`,
      );
  ...
  process.env.SENTIMENT_MODE = 'accurate';
  console.log(
    '✅ Tests completed. DistilBERT is now the primary sentiment analysis method.',
  );
}
