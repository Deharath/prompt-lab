      ...
      <div className={compact ? 'space-y-2' : 'space-y-2'}>
        <div className="flex items-center justify-between">
          ...
          <input
            type="number"
            id={`${id}-number`}
            className={`border-border focus:border-primary focus:ring-primary bg-background min-w-0 touch-manipulation rounded border text-right shadow-sm focus:ring-1 ${
              compact
                ? 'min-h-[44px] w-16 px-2 py-1 text-sm'
                : 'min-h-[44px] w-16 px-2 py-1 text-sm'
            }`}
            value={inputValue}
        ...
        <input
          type="range"
          id={id}
          className={`w-full min-w-0 cursor-pointer touch-manipulation appearance-none rounded-lg bg-gray-200 ${
            compact ? 'h-2' : 'h-3'
          }`}
          min={min}
          max={max}
