...
try {
  ...
  const sampleText = "The quick brown fox jumps over the lazy dog. This is a simple test.";
  const freScore = readabilityInstance.fleschReadingEase(sampleText);
  
  console.log(`✅ text-readability-ts loaded in ${(readabilityEnd - readabilityStart).toFixed(2)}ms`);
...
try {
  ...
  console.log(`✅ vader-sentiment loaded in ${(vaderEnd - vaderStart).toFixed(2)}ms`);
...
try {
  ...
  console.log(`✅ wink-tokenizer loaded in ${(tokenEnd - tokenStart).toFixed(2)}ms`);
...
try {
  ...
  console.log(`✅ @xenova/transformers loaded in ${(transformersEnd - transformersStart).toFixed(2)}ms`);
  console.log(`   Sentiment result: ${JSON.stringify(result)}`);
} catch (error) {
  console.log(`❌ @xenova/transformers failed: ${error.message}`);
  console.log(`   This may require additional setup or environment configuration`);
}
